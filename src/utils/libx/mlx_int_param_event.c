...
int	mlx_int_param_KEY_PRESS(t_xvar *xvar, XEvent *ev, t_win_list *win)
{
  win->hooks[KEY_PRESS].hook(XkbKeycodeToKeysym(xvar->display,
					       ev->xkey.keycode, 0, 0),
			    win->hooks[KEY_PRESS].param);
}

int	mlx_int_param_KEY_RELEASE(t_xvar *xvar, XEvent *ev, t_win_list *win)
{
  win->hooks[KEY_RELEASE].hook(XkbKeycodeToKeysym(xvar->display,
						 ev->xkey.keycode, 0, 0),
			      win->hooks[KEY_RELEASE].param);
}
...
int	(*(mlx_int_param_event[]))() =
{
  ...
  mlx_int_param_undef,
  mlx_int_param_KEY_PRESS,
  mlx_int_param_KEY_RELEASE,  /* 3 */
